...
resource "google_project_iam_member" "datastore_user" {
  ...
  #  https://github.com/terraform-providers/terraform-provider-google/pull/1197 
  # Fixed some parts of the timing issue, but the propagation of permissions to 
  # Datastore is not fixed
  provisioner "local-exec" {
...
resource "kubernetes_pod" "test" {
    ...
    container {
      ...
      env = [{
      ...
      },
        {
          ...
          value = "${kubernetes_namespace.todo_backend.metadata.0.name}-${var.version}"
        },
        {
  ...
  provisioner "local-exec" {
    command = "gcloud --project ${var.google_project} container clusters get-credentials ${data.terraform_remote_state.gke.name}" // To ensure that we can connect with kubectl outside terraform directly afterwards
  }
}
